"""
Script para enviar mensajes directamente a SQS (proforma-checkpoints) para pruebas de carga.

Configuración:
- config.py (raíz): Configuración general (ambiente, cantidad de mensajes, orderId, etc.)
- dev/config.py o qa/config.py: Configuración específica del ambiente (queue URL, región)

Destino: solo SQS (queue-finmg-proforma-checkpoints). El consumer bx-cnsr-finmg-proforma-checkpoints
lee message.body(), lo parsea como MessageSQS y extrae el JSON string de CheckpointEvent
desde messageSQS.getMessage().

Estrategia orderId fijo: todos los mensajes (sean de plantilla o sintéticos) llevan el
mismo orderId de prueba. Al terminar solo queda un documento en Mongo que limpiar.
"""
...
if _env_file.exists() and (
    not os.getenv("AWS_REGION") or not os.getenv("AWS_ACCOUNT_ID")
):
...
spec_general = importlib.util.spec_from_file_location(
    "config_general", general_config_path
)
...
_order_id_default = getattr(config_general, "ORDER_ID", "stress-load-test")
_send_mode_default = getattr(config_general, "SEND_MODE", "parallel").lower()
...
if sys.stdin.isatty():
    ...
    _r = input(f"orderId de prueba [{_order_id_default}]: ").strip()
    ORDER_ID = _r if _r else _order_id_default
    _r = (
        input(f"Modo de envío (parallel/sequential) [{_send_mode_default}]: ")
        .strip()
        .lower()
        or _send_mode_default
    )
    SEND_MODE = _r if _r in ("parallel", "sequential") else _send_mode_default
    print()
else:
    ENVIRONMENT = _env_default
    MAX_MESSAGES = _max_default
    ORDER_ID = _order_id_default
    SEND_MODE = _send_mode_default
...
if not CONFIG_QUEUE_URL:
    raise ValueError(
        "QUEUE_URL no definido. Revisa que AWS_REGION y AWS_ACCOUNT_ID estén en .env."
    )
...
# Importar builder
builder_path = script_dir / "checkpoint_event_builder.py"
if not builder_path.exists():
    raise FileNotFoundError(
        f"No se encontró checkpoint_event_builder.py en {script_dir}"
    )
spec_builder = importlib.util.spec_from_file_location(
    "checkpoint_event_builder", builder_path
)
...
async def main_async() -> None:
    ...
    if payloads:
        ...
        print(f"  orderId: {p0.get('orderId')}")
        print(f"  eventCode: {p0.get('eventCode')}")
        print(f"  trackingId: {p0.get('trackingId')}")
        print(f"  Message es string: {isinstance(env0.get('Message'), str)}")
        print(
            f"  MessageAttributes.channel: {env0.get('MessageAttributes', {}).get('channel', {}).get('Value')}"
        )
        print(
            f"  MessageAttributes.eventType: {env0.get('MessageAttributes', {}).get('eventType', {}).get('Value')}"
        )
        print("===============================\n")
    ...
    if SEND_MODE == "sequential":
        ok_count, error_count = await send_one_by_one(
            publisher, payloads, DELAY_MS, verbose=True
        )
    elif len(payloads) > BATCH_SIZE:
        ok_count, error_count = await send_in_batches(
            publisher, payloads, BATCH_SIZE, verbose=(len(payloads) <= 50)
        )
    else:
        ok_count, error_count = await send_one_by_one(
    ...
    order_ids_sent = [p.get("orderId", "") for p in payloads]
    log_file = save_log(order_ids_sent, ok_count, error_count)
...
def print_configuration() -> None:
    print("=" * 60)
    print("=== CONFIGURACIÓN (proforma-checkpoints SQS) ===")
    print("=" * 60)
    print(f"   • Ambiente: {ENVIRONMENT}")
    print(f"   • Máximo de mensajes: {MAX_MESSAGES}")
    print("   • Destino: SQS")
    print(
        f"   • Entidad (plantilla): {f'{ENVIRONMENT}/{INPUT_FILE}' if INPUT_FILE else 'ninguna (sintéticos)'}"
    )
    if ENTITY_PATH:
        print(f"   • Ruta plantilla: {ENTITY_PATH}")
    print(f"   • orderId de prueba: {ORDER_ID}")
    print(f"   • Tipo entidad: {ENTITY_TYPE}")
    print(f"   • Tipo evento: {EVENT_TYPE}")
    print(f"   • Cola SQS: {QUEUE_URL}")
    print(f"   • Región: {REGION}")
    print(f"   • Modo de envío: {SEND_MODE}")
    print(
        f"   • Delay: {DELAY_MS}ms | Lote: {BATCH_SIZE} | Concurrencia: {MAX_CONCURRENT}"
    )
        ...
        for item, result in zip(batch, results):
            if result.get("status") == "OK":
                ...
                if verbose or error_count <= 5:
                    print(
                        f"  ERROR - orderId {item.get('orderId', '')}: {result.get('error')}"
                    )
        if batch_idx <= 3 or batch_idx % 10 == 0 or batch_idx == total_batches:
            batch_end = min(batch_idx * batch_size, total)
            print(
                f"[Lote {batch_idx}/{total_batches}] Total: {batch_end}/{total} | OK: {ok_count} | ERROR: {error_count}"
            )
...
async def send_one_by_one(
    ...
    for idx, item in enumerate(items, 1):
        ...
        if result.get("status") == "OK":
            ...
            if verbose:
                print(
                    f"  [{idx}/{total}] OK - orderId: {item.get('orderId', '')[:50]}  eventCode: {item.get('eventCode', '')}"
                )
        else:
            error_count += 1
            print(
                f"  [{idx}/{total}] ERROR - orderId {item.get('orderId', '')}: {result.get('error')}"
            )
...
def save_log(order_ids: List[str], ok_count: int, error_count: int) -> str:
    """Guarda un log JSON con los orderIds enviados y resumen."""
    if not LOGS_DIR:
        return ""
    Path(LOGS_DIR).mkdir(parents=True, exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_file = str(Path(LOGS_DIR) / f"proforma_checkpoints_{timestamp}.json")
