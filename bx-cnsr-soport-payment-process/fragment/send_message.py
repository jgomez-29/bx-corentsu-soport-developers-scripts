"""
Script para enviar mensajes fragment a SNS (payment process, eventType paymentProcessRequested).
...
builder_path = script_dir / "payment_process_fragment_builder.py"
if not builder_path.exists():
    raise FileNotFoundError(f"No se encontró payment_process_fragment_builder.py en {script_dir}")
spec_builder = importlib.util.spec_from_file_location("payment_process_fragment_builder", builder_path)
...
async def main_async() -> None:
    print_configuration()
    print("Generando mensajes fragment (paymentProcessRequested)...")
    ...
    if payloads:
        ...
        print(f"bulkIdentifier: {p0.get('bulkIdentifier')}")
        print(f"eventType: paymentProcessRequested")
        print(f"documentsToCreate: {len(p0.get('documentsToCreate', []))} documentos")
        print("===============================\n")
...
def print_configuration():
    print("=" * 60)
    print("=== CONFIGURACIÓN (payment-process-fragment) ===")
    print("=" * 60)
...
async def send_one_by_one(publisher, items: List[Dict], envelope_builder_fn: Callable, delay_ms: int, verbose: bool) -> tuple:
    ...
    for idx, item in enumerate(items, 1):
        ...
        if result.get("status") == "OK":
            ok_count += 1
            if verbose:
                print(f"[{idx}/{total}] ✓ OK - bulkIdentifier: {item.get('bulkIdentifier', '')[:30]}...")
        else:
            error_count += 1
